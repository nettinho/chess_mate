defmodule ChessMate.Accounts do
  @moduledoc """
  ...
  alias ChessMate.Accounts.{User, UserToken, UserNotifier}

  defp then_preload({:ok, schema}, opts),
    do: {:ok, Repo.preload(schema, Keyword.get(opts, :preload, []))}
  ...
  def update_user(user, attrs, opts \\ []) do
    changeset = User.update_changeset(user, attrs, validate_email: false)
    ...
    |> Repo.transaction()
    |> case do
      {:ok, %{user: user}} -> {:ok, user} |> then_preload(opts)
      {:error, :user, changeset, _} -> {:error, changeset}
    end
  end
  ...
  """
  def update_user_role(%UserRole{} = user_role, attrs, opts \\ []) do
    user_role
